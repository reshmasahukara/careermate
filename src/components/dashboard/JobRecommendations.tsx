...
export default function JobRecommendations({ targetRole }: JobRecommendationsProps) {
  ...
  if (!targetRole) {
    return (
      <div className="flex flex-col items-center justify-center p-6 text-center border-2 border-dashed border-[#E5E7EB] rounded-[16px] bg-slate-50">
        ...
        <h4 className="text-[15px] font-semibold text-[#111827] mb-1">No Recommendations</h4>
  ...
  if (error || !data || !data.jobs || data.jobs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-6 text-center border-2 border-dashed border-[#E5E7EB] rounded-[16px] bg-slate-50">
        <h4 className="text-[15px] font-semibold text-[#111827] mb-1">No Jobs Found</h4>
        ...
        {jobs.map((job: any, i: number) => {
          ...
          return (
            <a
              ...
              className="group block p-4 bg-white hover:bg-[#F7F8FA] border border-[#E5E7EB] hover:border-[#14B8A6] rounded-[16px] transition-all flex flex-col h-full shadow-sm"
            >
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="w-10 h-10 rounded-xl bg-white border border-[#E5E7EB] flex items-center justify-center shrink-0 overflow-hidden">
                  {job.companyLogo ? (
              ...
              <div className="mb-auto">
                <p className="text-[15px] font-semibold text-[#111827] line-clamp-2 group-hover:text-[#14B8A6] transition-colors">
                  {job.title}
                </p>
                <p className="text-[13px] text-[#64748B] mt-1 truncate">{job.companyName}</p>
              </div>

              <div className="flex flex-wrap items-center gap-3 mt-4 text-[12px] text-[#64748B] font-medium pt-3 border-t border-[#E5E7EB]">
