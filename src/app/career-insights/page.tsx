      ...
      <div className="max-w-[1450px] mx-auto space-y-6 pb-20 print:p-0 print:space-y-4">
        ...
        {isNewUser ? (
          ...
          <>
            ...
            <div className="bg-white border border-slate-200 rounded-[24px] p-6 shadow-sm space-y-4 print:hidden">
              <div>
                <h3 className="text-base font-black text-[#0F172A] tracking-tight">Top Hiring Companies</h3>
                <p className="text-xs text-slate-500 font-semibold mt-0.5">Explore active roles in the database matching your targeting settings.</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {insights.companyCards.map((company: any, i: number) => (
                  <div key={i} className="border border-slate-200 rounded-2xl p-4 flex flex-col justify-between gap-4 bg-slate-50/50 hover:bg-white hover:shadow-md transition-all group">
                    <div className="flex gap-3">
                      ...
                      />
                      <div className="overflow-hidden">
                        <h4 className="font-bold text-sm text-slate-800 truncate">{company.title}</h4>
                        <p className="text-xs font-semibold text-slate-500 flex items-center gap-1 mt-0.5">
                          <Building2 className="w-3.5 h-3.5" /> {company.name}
                        </p>
                        <p className="text-[10px] text-slate-400 font-bold flex items-center gap-1 mt-1">
                          <MapPin className="w-3 h-3 text-slate-300" /> {company.location}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex flex-wrap gap-1">
                        {company.requiredSkills.map((skill: string) => (
                          <span key={skill} className="bg-slate-100 text-slate-600 text-[9px] font-bold px-2 py-0.5 rounded border border-slate-200">
                            {skill}
                          </span>
                        ))}
                      </div>
                ...
                {insights.companyCards.length === 0 && (
                  <div className="col-span-full py-12 text-center border border-dashed border-slate-200 rounded-2xl bg-white">
                    <p className="text-sm font-semibold text-slate-400">No jobs matching your target role found in the database.</p>
                  </div>
                )}
              </div>
            </div>
