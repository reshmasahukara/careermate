...
import { generateQualitativeResumeInsights } from "@/lib/resumeInsights";
 ...
 */
export async function processResumeUploadAction(userId: string, formData: FormData, targetRole: string = "") {
  isDbConfigured();
  ...
  try {
    ...
    // 3. Generate qualitative insights
    const insights = await generateQualitativeResumeInsights(textContent, targetRole);
    ...
    return await prisma.resume.create({
      data: {
        ...
        qualitativeAnalysis: insights as any,
      },
    });
  } catch (e) {
