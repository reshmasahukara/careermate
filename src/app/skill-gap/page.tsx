  ...
  const renderContent = (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-[#64748B] text-xs font-semibold mt-1">Benchmark your skill proficiencies against target market expectations.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          {/* Target selection */}
          <div className="bg-white border border-[#E2E8F0] p-6 rounded-[20px] space-y-4 shadow-sm">
            <div>
              <h3 className="font-bold text-[#0F172A] text-xs uppercase tracking-wider">Select Focus Role</h3>
              <p className="text-[10px] text-[#64748B] mt-0.5 font-semibold">Benchmarks adjust according to the selected role expectations.</p>
            </div>

            <select
              value={targetRole}
              onChange={(e) => setTargetRole(e.target.value)}
              className="w-full bg-[#F8FAFC] border border-[#E2E8F0] rounded-[12px] py-2.5 px-3.5 text-xs focus:outline-none focus:border-[#2563EB] font-semibold text-[#0F172A]"
            >
          ...
          <div className="bg-white border border-[#E2E8F0] p-6 rounded-[20px] space-y-4 shadow-sm">
            <div>
              <h3 className="font-bold text-[#0F172A] text-xs uppercase tracking-wider">Add Profile Skill</h3>
              <p className="text-[10px] text-[#64748B] mt-0.5 font-semibold">Append tools or coding parameters to your profile catalog.</p>
            </div>
            ...
            <form onSubmit={handleAddSkill} className="space-y-3">
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className="flex-1 bg-[#F8FAFC] border border-[#E2E8F0] rounded-[12px] py-2.5 px-3.5 text-xs focus:outline-none font-semibold text-[#64748B] focus:bg-white transition-colors"
                >
                ...
                <button
                  type="submit"
                  className="bg-[#2563EB] hover:bg-[#1D4ED8] text-white font-bold py-2.5 px-4 rounded-[12px] text-xs transition-colors flex items-center gap-1.5 shadow-sm cursor-pointer"
                >
            ...
            <div className="space-y-2">
              <span className="text-[10px] font-bold text-[#64748B] uppercase tracking-wider">Your Active Skills</span>
              {isLoadingSkills ? (
                ...
                <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto pr-1 no-scrollbar">
                  {skills.map((s) => (
                    <span
                      key={s.id}
                      className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-[8px] pl-2.5 pr-1.5 py-1 text-xs font-bold text-[#0F172A] flex items-center gap-1.5"
                    >
                      {s.name}
                      <span className="text-[9px] text-[#64748B] font-normal uppercase">({s.proficiency.charAt(0)})</span>
                      <button
                        type="button"
                ...
                <div className="text-[11px] text-[#64748B] italic font-semibold">No skills listed yet. Add one above.</div>
              )}
            </div>
          </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-white border border-[#E2E8F0] p-6 rounded-[20px] shadow-sm hover:shadow-md transition-all duration-200">
            <div>
              <h3 className="font-bold text-[#0F172A] text-base">Expectations Benchmark</h3>
              <p className="text-[#64748B] text-xs font-semibold">Comparing required proficiencies against your parameters.</p>
            </div>
            ...
            <div className="h-64 w-full mt-6">
              {isMounted ? (
                ...
                <div className="h-full bg-[#F8FAFC] border border-[#E2E8F0] rounded-[12px]" />
              )}
            </div>
          </div>
          ...
          <div className="bg-white border border-[#E2E8F0] p-6 rounded-[20px] space-y-4 shadow-sm hover:shadow-md transition-all duration-200">
            <div>
              <h3 className="font-bold text-[#0F172A] text-base">Identified Gaps</h3>
              <p className="text-[#64748B] text-xs font-semibold">Technologies to learn to match target requirements.</p>
            </div>

            {skillGaps.length > 0 ? (
              <div className="space-y-3.5">
                {skillGaps.map((gap) => (
                  <div key={gap.name} className="space-y-1.5 bg-[#F8FAFC] border border-[#E2E8F0] p-3 rounded-[12px]">
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-bold text-[#0F172A]">{gap.name}</span>
                      <span className="text-rose-500 font-extrabold text-[9px] uppercase bg-rose-50 px-2 py-0.5 rounded-full">
                        Gap: {gap.gap}%
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-[10px] text-[#64748B] font-bold uppercase tracking-wider">
                      ...
                      <span className="text-[#2563EB] hover:underline cursor-pointer flex items-center gap-0.5">
                ...
                <div className="pt-4 border-t border-[#E2E8F0]/60 flex justify-end">
                  <button
                    ...
                    className="bg-[#2563EB] hover:bg-[#1D4ED8] text-white font-bold py-3 px-6 rounded-[12px] text-xs flex items-center justify-center gap-1.5 shadow-sm transition-colors cursor-pointer"
                  >
                    {isGenerating ? (
                      <>
              ...
              <div className="p-4 bg-emerald-50 border border-emerald-100 text-emerald-850 rounded-[12px] text-xs flex items-center gap-2 font-semibold">
